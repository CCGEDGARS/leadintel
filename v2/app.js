...
const MANDATORY_LV_SUBJECT="Edgars Untāls · CCGROUP · {{company}}";
const MANDATORY_LV_BODY=`Sveicināti, {{first_name}}!\n\nTeikšu kā ir: šo e-pastu Jums nosūtīja Roberts, mans MI asistents. Sistēma pamanīja, ka {{company}} {{signal}}, un iespējams, mums šeit varētu izveidoties lieliska sadarbība.\n\nMani sauc Edgars Untāls. Esmu pārdošanas treneris un MI arhitekts. Palīdzu uzņēmumiem attīstīt pārdošanas komandas, automatizēt procesus, piesaistīt jaunus klientus un pelnīt naudu.\n\nMans LinkedIn profils:\nhttps://www.linkedin.com/in/edgarsuntals\n\n“Edgar, tas, ko Tu man šobrīd rādi, ir no nākamā gadsimta. Es zināju, ka tas kādreiz notiks, bet nezināju, ka tik ātri,” pagājušajā nedēļā man teica kāds klients.\n\nLai vai kā, šis stāsts nav par mani. Tas ir par {{company}} un jaunā laikmeta iespējām.\n\n{{first_name}}, kā būtu ar īsu 30 minūšu sarunu Zoom platformā?\n\nNodemonstrēšu konkrētus piemērus, kā strādā jaunākās tehnoloģijas un kā tās var palīdzēt sasniegt arī Jūsu biznesa mērķus.\n\nPiemērotāko laiku sarunai aicinu izvēlēties šeit:\n\nhttps://calendly.com/edgars-7go/strategy-call\n\nUz tikšanos!\n\nCieņā,\n\nEdgars Untāls\nBiznesa treneris · MI arhitekts`;
const MANDATORY_EN_SUBJECT="Edgars Untāls · CCGROUP · {{company}}";
const MANDATORY_EN_BODY=`Hello {{first_name}},\n\nLet me be direct: this email was sent to you by Roberts, my AI assistant. The system noticed that {{company}} {{signal}}, and it suggested that there may be a great opportunity for us to work together.\n\nMy name is Edgars Untāls. I am a sales trainer and AI architect. I help companies develop sales teams, automate business processes, attract new clients and grow revenue.\n\nMy LinkedIn profile:\nhttps://www.linkedin.com/in/edgarsuntals\n\n“Edgar, what you are showing me feels like it comes from the next century. I knew it would happen one day, but I did not know it would happen so quickly,” a client told me last week.\n\nEither way, this story is not about me. It is about {{company}} and the opportunities created by a new era of technology.\n\n{{first_name}}, would you be open to a brief 30-minute conversation on Zoom?\n\nI will show you practical examples of how the latest technologies work and how they could help {{company}} move towards its business goals.\n\nYou can choose the most convenient time here:\n\nhttps://calendly.com/edgars-7go/strategy-call\n\nSpeak soon!\n\nBest regards,\n\nEdgars Untāls\nBusiness Trainer · AI Architect`;
const DEFAULT_PLAYBOOKS=[
  {id:"sales-hiring-book",name:"Mandatory LV · Roberts introduction",offerId:"digital-sales-book",signalId:"sales-hiring",role:"Sales leader",channel:"Email",language:"Latvian",sendMode:"Approval required",ctaMode:"Include in initial email",minScore:8,dailyLimit:3,requireHighConfidence:true,autoApproved:false,subject:MANDATORY_LV_SUBJECT,body:MANDATORY_LV_BODY,active:true},
  {id:"crm-ai-integration",name:"Mandatory LV · Roberts introduction",offerId:"ai-sales-integration",signalId:"crm-ai",role:"Commercial or digital leader",channel:"Email",language:"Latvian",sendMode:"Approval required",ctaMode:"Include in initial email",minScore:8,dailyLimit:3,requireHighConfidence:true,autoApproved:false,subject:MANDATORY_LV_SUBJECT,body:MANDATORY_LV_BODY,active:true}
];
...
function loadState(){
  try{
    ...
    return {
      ...
      playbooks:normalizeArray(saved.playbooks,DEFAULT_PLAYBOOKS).map(item=>({...item,language:"Latvian",sendMode:"Approval required",autoApproved:false,subject:MANDATORY_LV_SUBJECT,body:MANDATORY_LV_BODY})),
...
function renderContacts(){
  ...
  document.getElementById("contacts-body").innerHTML=list.map(o=>{const linkedinTargets=linkedInContactTargets(o);const canWrite=isVerifiedWorkEmail(o);const linkedinConfirmed=canWrite;const needsLookup=!["Verified","Strong match"].includes(o.contact.emailStatus);return `<tr>
    ...
    <td><strong>${esc(o.signalType||"Market signal")}</strong><br><small>${esc(o.signal||"Signal details pending")}</small>${o.source_url?`<br><a class="evidence" href="${esc(o.source_url)}" target="_blank" rel="noopener">Source evidence ↗</a>`:""}</td>
    <td>${o.website?`<a class="evidence" href="${esc(o.website)}" target="_blank" rel="noopener">${esc(o.domain||"Open website")} ↗</a>`:'<span class="status warn">Website needed</span>'}</td>
...
function integrationDirectory(){
  ...
  return [
    ...
    {id:"github",name:"GitHub + Vercel",status:"Production via Vercel",tone:"good",url:"https://github.com/CCGEDGARS/leadintel",purpose:"GitHub stores source and CI history while Vercel hosts the production front-end.",setup:"Merge verified changes to main; Vercel publishes leadintel.ccgroup.lv/customer/.",credential:"GitHub and Vercel account access; no credentials are stored in this app."},
...
document.addEventListener("click",async event=>{
  ...
  if(writeContact){
    ...
    if(!isVerifiedWorkEmail(opportunity)){
      showToast("Write is locked until the contact has a verified work email");
      return;
    }
    const assistant=writingAssistantSettings();
