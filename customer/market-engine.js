(function(root,factory){
  ...
  const RESEARCH_CATEGORY_INTENTS=Object.freeze({
    direction:["official statistics sector output orders exports growth", "employment vacancies skills demand trend", "investment expansion permits business activity", "business formation insolvency closures trend", "industry outlook forecast demand"],
    competition:["competitors market share industry leaders", "supplier landscape positioning differentiation", "tender participation competing providers", "market concentration underserved segments"],
    funding:["EU Funding and Tenders Portal active calls eligibility deadline", "European Commission grants programmes active calls", "national funding agency grants loans guarantees", "regional development funding active programmes"],
  ...
  function buildResearchPlan(profile={},signals=[],input={mode:"quick"}){
    ...
    return planned.slice(0,RESEARCH_MODES[mode].maxQueries);
  }
  ...
  return {DEFAULT_MARKET_STATE,RESEARCH_MODES,SOURCE_TYPES,researchRuntimePolicy,effectiveResearchMarkets,buildIcpCandidates,normalizeSignals,addCustomSignal,filterResearchSourceTypes,buildResearchQueries,buildResearchPlan,buildResearchRecommendations,buildSuggestedSources,normalizeSearchResults,mergeResearchResults,buildMarketOpportunities,localizeGeneratedState,normalizeMarketState,recoverInterruptedResearch,withTimeout,mapWithConcurrency,normalizeMonitoring,appendResearchHistory,getMarketJourneyState,splitList};
});
