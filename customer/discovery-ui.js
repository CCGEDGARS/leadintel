...
const ASSET_VERSION="20260925-pipeline-choice-v1&sidebar-preservation=1&target-segments=1&target-quality=1";
...
async function runCompanyDiscovery({targetOnly=false}={}){
  const main=mainState();
  ...
  const baseQueries=targetOnly?[]:LeadIntelDiscovery.buildDiscoveryQueries(profileForQueries,market,limits.queryCount,discovery.queries);
  ...
  const queries=targetOnly?targetEvidenceQueries(researchTargets,targetMarket,market):[...baseQueries,...targetEvidenceQueries(researchTargets,targetMarket,market),...opportunityHypothesisQueries(main,targetMarket)];
  ...
  recoveredInterruptedRun=false;discovery.needsRefresh=false;discovery.queries=queries;discovery.rawResults=[];discovery.candidates=previousCandidates;discovery.companyMentions=[];discovery.searchFailures=[];discovery.providerFallbacks=[];discovery.checkedCompanyDomains=[];discovery.latestRunCandidateCount=0;discovery.lastSuccessfulRunAt=previousRunAt;discovery.retainedLastSuccessfulResults=previousCandidates.length>0;discovery.extraction=targetOnly?{status:"targets",method:"Target list",message:"Using your saved target names; checking their public evidence."}:{status:"pending",method:"",message:"Company-name extraction will report its method after the evidence search."};discovery.potentialMatches=[];
  ...
  try{
    const allSearches=(async()=>{
      ...
      const firstMentions=targetOnly?[]:await extractCompaniesFromEvidence(firstPass,targetMarket,Math.min(targetCount*3,MAX_DISCOVERY_COMPANY_CHECKS),runController.signal);
      ...
      if(!targetOnly&&firstPassSucceeded&&runCandidates.length<targetCount&&companyMentions.length<MAX_DISCOVERY_COMPANY_CHECKS){
...
function renderTargetList(){const node=$("discovery-target-list");if(!node)return;const items=selectedTargets(),researched=loadMeta().lastTargetResearchNames||[];node.innerHTML=`<div class="discovery-target-heading"><div><strong>Target Companies · ${items.length}</strong><p>Your chosen prospects. LeadIntel checks up to five per search for market fit and public signals; selection alone does not create a score.</p></div><button type="button" class="secondary-btn small" id="edit-target-companies">${items.length?'Edit targets':'Add targets'}</button></div>${items.length?`<div class="discovery-target-chips">${items.map(item=>{const qualified=discovery.candidates.find(candidate=>canonicalDomain(candidate.domain)===item.domain||(candidate.company||'').toLowerCase()===item.companyName.toLowerCase());const review=(discovery.potentialMatches||[]).find(candidate=>canonicalDomain(candidate.domain)===item.domain||(candidate.company||'').toLowerCase()===item.companyName.toLowerCase());return `<span><b>${esc(item.companyName)}</b> · ${qualified?`Qualified · ${qualified.score.total}/100`:review?'Review evidence':researched.includes(item.companyName.toLowerCase())?'No verified opportunity yet':'Research pending'}</span>`;}).join('')}</div>`:''}`;if(targetListHandoff){const main=mainState(),provisional=!main.profile||!main.approved||!main.market?.strategyApproved;node.insertAdjacentHTML('afterbegin',`<div class="discovery-target-handoff" role="status"><strong>You are now in Step 4 · Companies</strong>Your ${items.length} saved target ${items.length===1?'company is':'companies are'} listed below. ${discovery.status==='running'?'Research is running; follow the progress below.':discovery.lastRunAt?'Review the evidence and scores below.':'Press <b>Find companies</b> to research market fit and public demand signals.'}${provisional?'<small>Research can proceed with the current context. Review Strategy later to refine the ranking.</small>':''}</div>`);}node.querySelector('#edit-target-companies')?.addEventListener('click',()=>{if(window.LeadIntelReferenceCustomerLauncher?.open)void window.LeadIntelReferenceCustomerLauncher.open('targets');else window.LeadIntelReferenceCustomerUI?.open?.('targets');});}
...
function selectedProspects(){return (discovery.selectedProspects||[]).filter(item=>{const company=crmCompanyByDomain(item.domain);const inLocalPipeline=(discovery.pipeline||[]).some(row=>canonicalDomain(row.domain||row.website)===canonicalDomain(item.domain));return !inLocalPipeline&&(!crmAvailable||Boolean(company&&company.lifecycle_status!=="suppressed"&&!company.pipeline_stage));});}
function renderSelectedProspects(prospects){if(!prospects.length)return "";return `<section class="selected-prospect-list"><h3>Selected prospects · buying signal unconfirmed</h3><p>These companies passed the website, market and fit checks. You can find buyers, or explicitly add one to Pipeline as a user-selected opportunity. No buying signal or opportunity score is claimed.</p>${prospects.map(candidate=>{const domain=canonicalDomain(candidate.domain);return `<div class="selected-prospect-row"><div><strong>${esc(candidate.company||domain)}</strong><small>${esc(domain)} · ${esc(candidate.market||"Target market")} · Buying signal unconfirmed</small>${candidate.people?.length?`<small>${candidate.people.map(person=>`${esc(person.name)} · ${esc(person.title)}`).join("; ")}</small>`:""}${candidate.peopleStatus==="error"?"<small>Buyer search failed · retry available</small>":""}</div><div class="selected-prospect-actions"><button class="secondary-btn small" type="button" data-find-prospect-buyers="${esc(domain)}" ${candidate.peopleStatus==="loading"?"disabled":""}>${candidate.people?.length?"Refresh buyers":candidate.peopleStatus==="loading"?"Searching…":"Find decision-makers"}</button><button class="primary-btn small" type="button" data-add-prospect-pipeline="${esc(domain)}" ${crmAuthenticated()?"":"disabled"}>Add to Pipeline · signal unconfirmed</button></div></div>`;}).join("")}</section>`;}
...
function bindDiscovery(){
  ...
  window.addEventListener("leadintel:open-discovery",event=>{targetListHandoff=event.detail?.source==='target-companies';if(!moduleReady()){showToast('Add your company website first');return;}showDiscoveryStep(event.detail?.focus||"companies");if(event.detail?.startResearch&&targetListHandoff&&discovery.status!=='running')void runCompanyDiscovery({targetOnly:true});});window.addEventListener("leadintel:module-opened",event=>{if(Number(event.detail?.step)!==5)return;syncStrategyFingerprint();saveMeta({...loadMeta(),visibleStep:5});renderAll();renderDiscoveryFocus(loadMeta().activeJourneyStage===5?"buyers":"companies");if(crmAuthenticated())refreshCrmState();});
