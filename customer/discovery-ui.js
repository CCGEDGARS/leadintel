...
function renderTargetList(){const node=$("discovery-target-list");if(!node)return;const items=selectedTargets(),researched=loadMeta().lastTargetResearchNames||[];node.innerHTML=`<div class="discovery-target-heading"><div><strong>Target Companies · ${items.length}</strong><p>Your chosen prospects. LeadIntel checks up to five per search for market fit and public signals; selection alone does not create a score.</p></div><button type="button" class="secondary-btn small" id="edit-target-companies">${items.length?'Edit targets':'Add targets'}</button></div>${items.length?`<div class="discovery-target-chips">${items.map(item=>{const qualified=discovery.candidates.find(candidate=>canonicalDomain(candidate.domain)===item.domain||(candidate.company||'').toLowerCase()===item.companyName.toLowerCase());const review=(discovery.potentialMatches||[]).find(candidate=>canonicalDomain(candidate.domain)===item.domain||(candidate.company||'').toLowerCase()===item.companyName.toLowerCase());return `<span><b>${esc(item.companyName)}</b> · ${qualified?`Qualified · ${qualified.score.total}/100`:review?'Review evidence':researched.includes(item.companyName.toLowerCase())?'No verified opportunity yet':'Research pending'}</span>`;}).join('')}</div>`:''}`;node.querySelector('#edit-target-companies')?.addEventListener('click',()=>{if(window.LeadIntelReferenceCustomerLauncher?.open)void window.LeadIntelReferenceCustomerLauncher.open('targets');else window.LeadIntelReferenceCustomerUI?.open?.('targets');});}
