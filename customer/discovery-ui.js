...
const ASSET_VERSION="20260925-ranked-prospects-v1";
...
function selectedProspects(){return (discovery.selectedProspects||[]).filter(item=>{const company=crmCompanyByDomain(item.domain);return !crmAvailable||Boolean(company&&company.lifecycle_status!=="suppressed"&&!company.pipeline_stage);});}
function renderSelectedProspects(prospects){if(!prospects.length)return "";return `<section class="selected-prospect-list"><h3>Selected prospects · buying signal unconfirmed</h3><p>Verified-fit companies selected for buyer research. They remain outside the opportunity Pipeline.</p>${prospects.map(candidate=>{const domain=canonicalDomain(candidate.domain);return `<div class="selected-prospect-row"><div><strong>${esc(candidate.company||domain)}</strong><small>${esc(domain)} · ${esc(candidate.market||"Target market")}</small>${candidate.people?.length?`<small>${candidate.people.map(person=>`${esc(person.name)} · ${esc(person.title)}`).join("; ")}</small>`:""}${candidate.peopleStatus==="error"?"<small>Buyer search failed · retry available</small>":""}</div><button class="primary-btn small" type="button" data-find-prospect-buyers="${esc(domain)}" ${candidate.peopleStatus==="loading"?"disabled":""}>${candidate.people?.length?"Refresh buyers":candidate.peopleStatus==="loading"?"Searching…":"Find decision-makers"}</button></div>`;}).join("")}</section>`;}
