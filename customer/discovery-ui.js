...
async function runCompanyDiscovery(){
  ...
  const queries=LeadIntelDiscovery.buildDiscoveryQueries(main.profile||{website:main.website},market,limits.queryCount,discovery.queries);
...
function renderCandidates(){const target=$("company-candidates");if(!target)return;
  ...
  return;
    ...
    <p class="candidate-narrative"><strong>Why this company:</strong> ${esc(c.market||"Target market")} presence${c.fitReasons?.length?` · strategy terms in evidence: ${esc(c.fitReasons.join(", "))}`:""}${c.lookalikeMatch?.reasons?.length?` · resembles approved customer examples: ${esc(c.lookalikeMatch.reasons.slice(0,2).join("; "))}`:""} · ${c.matchedSignals?.length?`public signal: ${esc(c.matchedSignals.map(s=>s.name).join(", "))}`:"signal not confirmed"}. Word matches and resemblance do not confirm a purchase. Review source links before selecting buyers.</p>
