...
async function firecrawlCompanySearch(queryMeta,runSignal){
  ...
  for(let attempt=0;attempt<=(activeDiscoverySavingMode?0:DISCOVERY_PROVIDER_RETRIES);attempt++){
    ...
    const timeout=setTimeout(()=>controller.abort(),DISCOVERY_REQUEST_TIMEOUT_MS);
    try{
      if(activeDiscoverySavingMode&&discoveryFirecrawlCalls>=6)throw new Error("Saving Mode search limit reached. Review current evidence before a larger run.");
      discoveryFirecrawlCalls++;
      const body={query:queryMeta.query,limit:activeDiscoverySavingMode?queryMeta.kind==='verification'?2:3:MAX_DISCOVERY_RESULTS_PER_QUERY};
...
async function runCompanyDiscovery({targetOnly=false,savingMode=false,targetDomain=""}={}){
  ...
  const baseQueries=targetOnly?[]:LeadIntelDiscovery.buildDiscoveryQueries(profileForQueries,market,savingMode?1:limits.queryCount,discovery.queries);
  ...
  const researchTargets=orderedTargets.slice(0,savingMode?1:5);
  ...
  try{
    const allSearches=(async()=>{
      ...
      let linkedEvidence=await collectResolutionAndVerification(companyMentions,savingMode?2:MAX_DISCOVERY_COMPANY_CHECKS);
