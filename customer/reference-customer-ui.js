...
(function installReferenceCustomerUI(root){
  ...
  function ensureModal(){
    ...
    modal.innerHTML=`<div class="reference-customer-backdrop" data-reference-close></div><section class="reference-customer-dialog" role="dialog" aria-modal="true" aria-labelledby="reference-customer-title">
      ...
      <section id="reference-segment-review" class="reference-segment-review" hidden><div class="reference-section-head"><div><span class="eyebrow">Customer segments</span><h3>Review the patterns LeadIntel found</h3></div><span id="reference-analysis-count"></span></div><div id="reference-segments"></div><p class="reference-activation-note"><strong>Activation means priority modeling.</strong> Selected segments influence Discovery queries and company ranking in the Step 1 target market. It does not add these companies to outreach.</p></section>
  ...
  function render(){
    ...
    const dna=reference.dna;modal.querySelector('#reference-dna-summary').innerHTML=dna?`<div class="reference-active-model"><span class="eyebrow">Priority model active</span><strong>${dna.activeCount} reference customers · ${esc(dna.confidence)} confidence</strong><p>Discovery will prioritize companies that match these patterns in ${(state.targetMarkets||[]).join(' · ')||'your selected market'}.</p><div>${(dna.dimensions||[]).slice(0,7).map(d=>`<span><b>${esc(d.key)}</b> ${esc((d.values||[]).join(', '))}</span>`).join('')}</div></div>`:'';
  ...
  function renderSegments(reference){const section=document.getElementById('reference-segment-review'),container=document.getElementById('reference-segments'),count=document.getElementById('reference-analysis-count');if(!section||!container)return;const segments=reference.segments||[];section.hidden=!segments.length;if(!segments.length){container.innerHTML='';return;}count.textContent=`${Object.keys(reference.analyses||{}).length} analyzed`;
    const notice=reference.segmentationMeaningful?'':`<div class="reference-coherent-note"><strong>No meaningful sub-segments detected.</strong><span>The uploaded customers form one coherent reference group.</span></div>`;
