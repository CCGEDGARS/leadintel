...
(function installReferenceCustomerUI(root){
  ...
  function selectedSegmentIds(){return [...document.querySelectorAll('[data-reference-segment]:checked')].map(node=>node.dataset.referenceSegment);}
  ...
  function render(){
    ...
    const dna=reference.dna;modal.querySelector('#reference-dna-summary').innerHTML=dna?`<div class="reference-active-model"><span class="eyebrow">Approved customer profile</span><strong>${esc(dna.profileName||'Reference customer model')} · ${dna.sampleSize||dna.activeCount} references · ${esc(dna.profileConfidence||dna.confidence)} confidence</strong>${dna.profileSummary?`<p>${esc(dna.profileSummary)}</p>`:''}<p>Discovery will use these observed traits as a soft preference in ${(state.targetMarkets||[]).join(' · ')||'your selected market'}.</p><div>${(dna.dimensions||[]).slice(0,7).map(d=>`<span><b>${esc(d.key)}</b> ${esc((d.values||[]).join(', '))}</span>`).join('')}</div></div>`:'';
  ...
  function renderSegments(reference){const section=document.getElementById('reference-segment-review'),container=document.getElementById('reference-segments'),count=document.getElementById('reference-analysis-count');if(!section||!container)return;const segments=reference.segments||[];section.hidden=!segments.length;if(!segments.length){container.innerHTML='';return;}count.textContent=`${Object.keys(reference.analyses||{}).length} analyzed`;
    const sampleSize=Object.keys(reference.analyses||{}).length;
    const notice=reference.segmentationMeaningful?'':`<div class="reference-coherent-note"><strong>${segments[0]?.confidence==='low'?'Early profile hypothesis':'One shared customer profile suggested'}</strong><span>Inferred from ${sampleSize} analyzed reference compan${sampleSize===1?'y':'ies'}. Check the category and traits before approving; one-company profiles are low confidence.</span></div>`;
    container.innerHTML=notice+segments.map(segment=>{const checked=reference.activeSegmentIds?.includes(segment.id)||(!reference.activated&&segments.length===1);return `<label class="reference-segment-card"><input type="checkbox" data-reference-segment="${esc(segment.id)}" ${checked?'checked':''}><div><div class="reference-segment-head"><strong>${esc(segment.name)}</strong><span>${segment.count} companies · ${esc(segment.confidence)} confidence</span></div>${segment.summary?`<p>${esc(segment.summary)}</p>`:''}<small>${(segment.traits||[]).slice(0,5).map(esc).join(' · ')}</small></div></label>`;}).join('');
  }
