...
const ASSET_VERSION="20260905-app-audit-v2";
...
async function confirmSent(){const pkg=currentPackage();if(!pkg){toast("Select an approved opportunity");return;}const sentAt=new Date().toISOString();const result=LeadIntelDelivery.confirmSend(delivery,pkg,q("delivery-recipient").value,sentAt);if(result.error){toast(result.error);return;}delivery=result.state;saveDelivery();updatePipelineStage(pkg.domain,"Contacted");markOutreachContacted(pkg.domain,result.record.sentAt);const handed=sessionStorage.getItem("leadintel_gmail_crm_activity_key")||"";if(handed)sessionStorage.removeItem("leadintel_gmail_crm_activity_key");const activityId=handed||crmActivityId("manual-send",pkg.domain,`${pkg.approvedAt||result.record.sentAt}-${result.record.recipientEmail}`);const crm=await syncCrmDelivery(pkg.domain,{activity:{id:activityId,type:"email.sent",channel:handed?"gmail":"gmail-compose",direction:"outbound",subject:pkg.drafts?.emailSubject||"",summary:`Email sent to ${result.record.recipientEmail}`,occurred_at:result.record.sentAt,metadata:{recipient:result.record.recipientEmail,delivery_channel:handed?"gmail-api":"gmail-compose"}},stage:"Contacted"});renderAll();toast(crm.ok?"Send confirmed · Pipeline moved to Contacted":"Send confirmed locally · CRM sync unavailable");}
