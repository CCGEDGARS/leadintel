...
function renderSelector(){const list=approvedPackages();const select=q("delivery-company-select");if(!select)return;select.innerHTML=list.length?list.map(item=>`<option value="${esc(item.domain)}" ${item.domain===delivery.selectedDomain?"selected":""}>${esc(item.company)} · ${esc(item.drafts?.tone||"consultative")}</option>`).join(""):'<option value="">No approved campaign packages yet</option>';const gate=q("continue-to-delivery");if(gate){gate.disabled=false;gate.textContent="Open Delivery & Learning →";}const record=currentRecord();q("delivery-recipient").value=record?.recipientEmail||"";q("delivery-recipient").disabled=!list.length;q("open-gmail-draft").disabled=!list.length;q("confirm-delivery-sent").disabled=!list.length;}
