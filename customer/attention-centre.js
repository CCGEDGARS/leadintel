(function(root){
  ...
  function render(){
    ...
    trigger.dataset.healthStatus=errors.length?'error':recommendations.length?'recommendation':'healthy';
    if(count){count.textContent=String(actionableItems.length);count.hidden=!actionableItems.length;}
    if(summary)summary.textContent=errors.length?`${errors.length} error${errors.length===1?'':'s'} to fix`:recommendations.length?`${recommendations.length} recommendation${recommendations.length===1?'':'s'}`:'Workspace healthy';
    const list=createDrawer().querySelector('.attention-list');list.replaceChildren();
    if(!actionableItems.length){const empty=document.createElement('div');empty.className='attention-empty is-healthy';empty.innerHTML='<strong>Workspace healthy</strong><span>No current errors or recommended improvements were detected.</span>';list.appendChild(empty);return;}
