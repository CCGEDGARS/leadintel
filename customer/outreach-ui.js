...
const LANGUAGE_ASSET_VERSION="20260916-brand-outreach-v5";
...
async function buildDossier(){
  ...
  item=LeadIntelOutreach.invalidateOutreachApproval({...item,dossier,researchStatus:research.length?(failures?"partial":"complete"):"error",researchAt:new Date().toISOString(),selectedPersonId:contact?.id||"",drafts:LeadIntelOutreach.buildOutreachDrafts(dossier,contact,main.profile||{},tone,language),contentLanguage:language});upsertItem(item);
...
function regenerateDrafts(){const current=currentItem();if(!current?.dossier)return;let item=LeadIntelOutreach.invalidateOutreachApproval(current);item.selectedPersonId=q("outreach-contact-select").value;item.drafts=LeadIntelOutreach.buildOutreachDrafts(item.dossier,selectedContact(item),mainState().profile||{},q("outreach-tone").value,contentLanguage());item.contentLanguage=contentLanguage();upsertItem(item);renderDossier();toast("Draft regenerated · approval required");}
...
async function approveOutreach(){const source=currentItem(),edited=readDraftEdits();if(!source?.dossier||!edited){toast("Build the dossier first");return;}const item=LeadIntelOutreach.approveOutreachItem(source,edited.drafts,new Date().toISOString(),{brandIdentity:mainState().brandIdentity});upsertItem(item);if(!item.approved){toast(item.error);renderDossier();return;}updatePipelineStage(item.domain,"Ready for Outreach");const candidate=selectedCandidate()||{domain:item.domain,company:item.company,stage:"Ready for Outreach"};const crm=await syncCrmActivity(candidate,{activity:{id:crmActivityId("content-approved",item.domain,item.approvedAt),type:"content.approved",summary:"Outreach content package approved",occurred_at:item.approvedAt,metadata:{tone:item.drafts?.tone||"",selected_person_id:item.selectedPersonId||"",brand_revision:item.brandSnapshot?.revision||null}},stage:"Ready for Outreach"});renderDossier();window.dispatchEvent(new CustomEvent("leadintel:outreach-approved",{detail:{domain:item.domain,email:LeadIntelOutreach.buildApprovedSendPayload(item)}}));toast(crm.ok?"Content package approved · Pipeline is Ready for Outreach":"Content approved locally · CRM sync unavailable");}

function renderApprovedPreview(mode="html"){
  const item=currentItem(),email=LeadIntelOutreach.renderApprovedEmail(item);if(!email){toast("Approve the content package before previewing it");return;}
...
function renderDossier(){const item=currentItem();const workspace=q("dossier-workspace");if(!workspace)return;if(!item?.dossier){workspace.hidden=true;q("dossier-research-status").textContent=item?.researchStatus==="running"?"Researching official and public sources…":pipeline().length?"Choose a saved company and build its dossier.":"No saved company yet. Use Discovery to create a pipeline opportunity when ready.";q("outreach-status").textContent=pipeline().length?"Ready":"Waiting for opportunity";return;}workspace.hidden=false;const d=item.dossier;q("dossier-research-status").textContent=item.researchStatus==="partial"?`Dossier built · ${d.evidence.length} sources · some research unavailable`:item.researchStatus==="error"?"Dossier built conservatively from existing evidence; deep research was unavailable.":`Dossier ready · ${d.evidence.length} evidence sources`;
  ...
  [q("outreach-contact-select"),q("outreach-tone"),q("outreach-email-subject"),q("outreach-email-body"),q("outreach-linkedin"),q("outreach-call-opener"),q("outreach-follow-up"),q("outreach-objection-reply")].forEach(el=>{if(el)el.disabled=Boolean(item.approved);});
  q("regenerate-outreach").disabled=false;q("regenerate-outreach").textContent=item.reapprovalRequired?"Regenerate & reapprove":item.approved?"Regenerate & reapprove":"Regenerate all";q("approve-outreach").disabled=Boolean(item.approved);q("approve-outreach").textContent=item.reapprovalRequired?"Review & reapprove":item.approved?"Approved ✓":"Approve content package";q("preview-approved-email").hidden=!item.approved;q("mark-contacted").disabled=!item.approved||Boolean(item.contactedAt);q("mark-contacted").textContent=item.contactedAt?"Contacted ✓":"Mark contacted";
  if(!item.approved)q("outreach-email-preview").hidden=true;
  q("outreach-approval-label").textContent=item.reapprovalRequired?"Reapproval required — branded email changed or could not be verified":item.contactedAt?`Contacted · ${new Date(item.contactedAt).toLocaleString()}`:item.approved?`Approved · ${new Date(item.approvedAt).toLocaleString()}`:"Draft not approved";q("outreach-approval-help").textContent=item.reapprovalRequired?"Sending is blocked. Regenerate this package, review the new email, then approve it again.":"Approval freezes the current email identity and moves the opportunity to Ready for Outreach. Sending remains a separate explicit action in Delivery & Learning.";q("outreach-status").textContent=item.reapprovalRequired?"Reapproval required":item.contactedAt?"Contacted":item.approved?"Approved":"Draft";q("outreach-status").classList.toggle("approved",Boolean(item.approved));
}
