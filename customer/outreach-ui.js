...
function injectOutreachUI(){
  ...
  if(pipelinePanel&&!q("continue-to-outreach"))pipelinePanel.insertAdjacentHTML("afterend",'<div class="outreach-entry"><div><span class="eyebrow">Next step</span><strong>Turn saved opportunities into evidence-backed conversations.</strong></div><button class="primary-btn" id="continue-to-outreach" type="button" disabled>Continue to Opportunity Dossiers →</button></div>');
  const content=document.querySelector("main.content");
  if(content&&!q("step-6"))content.insertAdjacentHTML("beforeend",`<section class="step-view" id="step-6" data-step="6">
    <div class="profile-header outreach-header"><div><span class="eyebrow">Step 6 · Opportunity Dossier & Outreach</span><h1>Turn evidence into a relevant conversation.</h1><p>Deep-research one saved company, separate evidence from hypotheses, choose the right buyer, then approve an editable outreach package. Nothing is sent automatically.</p></div><div class="profile-header-actions"><span class="profile-status" id="outreach-status">Draft</span><button class="secondary-btn small" id="back-to-discovery" type="button">← Discovery</button></div></div>
    <section class="panel dossier-selector"><div class="section-title"><span class="eyebrow">Select opportunity</span><h3>Choose a saved pipeline company</h3><p>Each dossier uses one official-site scrape and at most two targeted public searches.</p></div><div class="dossier-select-row"><select id="outreach-company-select"></select><button class="primary-btn" id="build-opportunity-dossier" type="button">Build dossier ✦</button></div><div class="research-status" id="dossier-research-status">Select a saved pipeline company.</div></section>
...
function showOutreachStep(){if(!pipeline().length){toast("Save at least one company to Pipeline first");return;}ensureSelection();renderAll();showStep(6);}
...
function renderSelector(){const list=pipeline();const select=q("outreach-company-select");if(!select)return;select.innerHTML=list.length?list.map(item=>`<option value="${esc(item.domain)}" ${item.domain===outreach.selectedDomain?"selected":""}>${esc(item.company)} · ${esc(item.stage)} · ${item.score?.total||0}/100</option>`).join(""):'<option value="">No saved companies</option>';const gate=q("continue-to-outreach");if(gate){gate.disabled=!list.length;gate.textContent=list.length?`Continue to Opportunity Dossiers →":"Save a company to Pipeline first"`;}}
function renderDossier(){const item=currentItem();const workspace=q("dossier-workspace");if(!workspace)return;if(!item?.dossier){workspace.hidden=true;q("dossier-research-status").textContent=item?.researchStatus==="running"?"Researching official and public sources…":"Build the selected company's dossier to continue.";q("outreach-status").textContent="Draft";return;}workspace.hidden=false;const d=item.dossier;q("dossier-research-status").textContent=item.researchStatus==="partial"?`Dossier built · ${d.evidence.length} sources · some research unavailable`:item.researchStatus==="error"?"Dossier built conservatively from existing evidence; deep research was unavailable.":`Dossier ready · ${d.evidence.length} evidence sources`;
