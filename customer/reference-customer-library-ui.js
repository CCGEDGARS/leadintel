...
(function installReferenceCustomerLibraryUI(root){
  ...
  function currentMessage({saved,analyzed,segments,selected,draftDirty,hasActivatableSegment,canSaveAndAnalyze}){
    ...
    if(!selected.active)return `<strong>${esc(selected.name)} · ${analyzed} analyzed</strong><span>${segments?`${segments} customer segment${segments===1?'':'s'} ready. Review below, then Activate Model.`:'Analysis complete. Review the results below.'}</span>`;
