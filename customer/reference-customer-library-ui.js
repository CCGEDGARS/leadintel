...
(function installReferenceCustomerLibraryUI(root){
  ...
  function injectStyles(){
    ...
    const style=document.createElement('style');style.id='reference-library-styles';style.textContent=`
      ...
      @media(max-width:900px){.reference-advanced-grid{grid-template-columns:1fr 1fr}.reference-library-top{flex-direction:column}.reference-current-name{grid-template-columns:1fr}.reference-flow-summary{grid-template-columns:1fr}}
  ...
  function renderSavedLists(state,editorHtml='',draftDirty=false){
    ...
    }).join('')}<div class="reference-activation-guidance ${portfolio.lists.some(list=>list.active&&list.reference?.publishedModel?.active)?'active':'pending'}" role="status">${portfolio.lists.some(list=>list.active&&list.reference?.publishedModel?.active)?'<strong>✓ Customer context is active in the flow</strong>LeadIntel will use this customer model to guide the next Company Discovery search. Your saved Target Companies and Opportunity Map are separate inputs; run Company Discovery to verify and rank prospects before finding decision makers.':portfolio.lists.some(list=>list.reference?.publishedModel?.active||(list.reference?.segments||[]).some(segment=>segment.canActivate!==false))?'<strong>Next: finish customer review</strong>Open View Results, check the suggested profile, and build the Opportunity Map for a single customer. Then select the profile and activate the model.':'<strong>Next: analyze your saved customer list</strong>Review the buyer context after analysis, then press Activate to include it in Company Discovery.'}</div></div>`;
  }
