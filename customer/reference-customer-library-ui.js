...
(function installReferenceCustomerLibraryUI(root){
  ...
  function injectStyles(){
    ...
    const style=document.createElement('style');style.id='reference-library-styles';style.textContent=`
      ...
      .reference-activation-steps button{margin-top:8px;border:0;background:none;padding:0;color:#0d684f;font-size:12px;font-weight:800;text-decoration:underline;text-underline-offset:3px;cursor:pointer}.reference-activation-steps button:focus-visible{outline:2px solid #0d684f;outline-offset:3px}
  ...
  document.addEventListener('click',event=>{
    ...
    const reviewProfile=event.target?.closest?.('[data-review-reference-profile]');if(reviewProfile){event.preventDefault();const id=snapshot().referenceCustomerPortfolio.selectedListId;if(id)void viewResults(id);return;}
    const openMap=event.target?.closest?.('[data-open-reference-map]');if(openMap){event.preventDefault();const id=snapshot().referenceCustomerPortfolio.selectedListId;if(id)void viewResults(id).then(()=>document.getElementById('customer-opportunity-map')?.scrollIntoView?.({behavior:'smooth',block:'start'}));return;}
    const researchTargets=event.target?.closest?.('[data-reference-research-targets]');if(researchTargets){event.preventDefault();root.LeadIntelReferenceCustomerUI?.close?.();root.dispatchEvent(new CustomEvent('leadintel:open-discovery',{detail:{focus:'companies',source:'target-companies',startResearch:true}}));return;}
