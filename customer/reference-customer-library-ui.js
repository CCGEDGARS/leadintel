...
(function installReferenceCustomerLibraryUI(root){
  ...
  function renderSavedLists(state){
    ...
    return `<div class="reference-saved-lists"><div class="reference-saved-head"><strong>Saved Lists</strong><span>${portfolio.lists.length} saved · ${portfolio.lists.filter(x=>x.active).length} active</span></div>${portfolio.lists.map(list=>{
      ...
      return `<div class="reference-saved-row ${list.active?'active':''} ${list.id===selectedId?'selected':''}" data-reference-list-row="${esc(list.id)}"><div class="reference-saved-info"><strong>${esc(list.name)}<span class="reference-list-state ${list.active?'active':''}">${stateLabel}</span></strong><small>${rows} customers · ${analyzed} analyzed${model?` · ${esc(model.confidence||'low')} confidence`:''}${list.markets?.length?` · ${esc(list.markets.join(' · '))}`:''}</small></div><div class="reference-saved-buttons"><button class="secondary-btn" type="button" data-open-reference-list="${esc(list.id)}">Open</button>${model?`<button class="${list.active?'secondary-btn':'primary-btn'}" type="button" data-toggle-reference-list="${esc(list.id)}" data-next-active="${list.active?'false':'true'}">${list.active?'Deactivate':'Activate'}</button>`:''}</div></div>`;
    }).join('')}</div>`;
  }
  ...
  root.LeadIntelReferenceCustomerLibraryUI={sync:syncLibraryUi,publishSelected,saveList,openList,createNewList,toggleList,activateCurrent,legacyLabels:LEGACY_LABELS};
})(globalThis);
