...
(function installReferenceCustomerLibraryUI(root){
  ...
  function syncLibraryUi(){
    ...
    panel.innerHTML=`<div class="reference-library-top"><div class="reference-library-main"><div class="reference-library-head"><strong>Past Customer List</strong><span class="reference-library-badge ${statusClass}">${esc(statusLabel)}</span></div><div class="reference-library-status">${portfolio.lists.length} saved list${portfolio.lists.length===1?'':'s'} · ${activeModels} active model${activeModels===1?'':'s'}. Only add customers who bought from you. One example is a low-confidence clue; no examples are required to search for opportunities. Upload → Save → Analyze → Review → Activate.</div></div><button class="secondary-btn" type="button" data-new-reference-list ${draftDirty?'disabled title="Save your changes before creating another list."':''}>+ New List</button></div>
