(function(root,factory){
  ...
  function recommendOffer(candidate={},profile={},research=[]){
    ...
    const corpus=[
      candidate.company,candidate.market,
      ...(candidate.matchedSignals||[]).flatMap(s=>[s.name,...(s.matchedTerms||[])]),
      ...(candidate.evidence||[]).flatMap(e=>[e.title,e.description,e.text]),
      ...(research||[]).flatMap(e=>[e.title,e.description,e.text])
    ].join(" ").toLowerCase();
    let best=offers[0],bestScore=-1;
    for(const offer of offers){
      const terms=unique(tokenize(offer));
  ...
  function evidenceFromCandidate(candidate={}){
    return (candidate.evidence||[]).map(item=>({
      url:normalizeUrl(item.url),title:clean(item.title)||clean(candidate.company),description:clean(item.description),text:clean(item.text||item.description).slice(0,6000),date:clean(item.date),sourceType:"Discovery"
    })).filter(item=>item.url);
  }
  function dedupeEvidence(items=[]){
    const map=new Map();
    for(const item of items){if(!item?.url)continue;const key=item.url.replace(/\/$/,"");if(!map.has(key))map.set(key,item);}
    return [...map.values()].slice(0,15);
  }
  function activeObservedSignals(candidate={},market={},evidence=[]){
    const inherited=(candidate.matchedSignals||[]).map(s=>({name:clean(s.name),weight:Number(s.weight)||0})).filter(s=>s.name);
    if(inherited.length)return inherited;
    const corpus=evidence.map(e=>`${e.title} ${e.description} ${e.text}`).join(" ").toLowerCase();
    return (market.signals||[]).filter(s=>s.active!==false).map(s=>{
      const terms=String(s.keywords||s.name||"").split(/;|,|\|/).map(clean).filter(Boolean);
      return terms.some(t=>corpus.includes(t.toLowerCase()))?{name:clean(s.name),weight:Number(s.weight)||0}:null;
  ...
  function buildOpportunityDossier(candidate={},profile={},market={},research=[]){
    ...
    const observed=activeObservedSignals(candidate,market,evidence);
    const recommendedOffer=recommendOffer(candidate,profile,research);
    const primarySignal=observed[0]?.name||"relevant commercial activity";
    const sourceCount=evidence.length;
    const whyNow=sourceCount
      ?`Public evidence for ${clean(candidate.company)||clean(candidate.domain)} currently includes material connected to ${primarySignal}. LeadIntel found ${sourceCount} source${sourceCount===1?"":"s"} in the dossier. This is a reason to test relevance now; it does not confirm buying intent.`
      :`No additional public evidence was available for ${clean(candidate.company)||clean(candidate.domain)}. Keep this opportunity in research rather than assuming active buying intent.`;
    const hypotheses=[];
    if(recommendedOffer)hypotheses.push(`Hypothesis: if the observed ${primarySignal} affects the approved buying context, ${recommendedOffer} may be worth discussing.`);
    if(clean(profile.differentiation))hypotheses.push(`Hypothesis: the seller's approved differentiation (${clean(profile.differentiation)}) may be relevant if it addresses the prospect's actual priorities; validate this in conversation.`);
    return {
      company:clean(candidate.company),domain:clean(candidate.domain)||domainOf(candidate.website),website:normalizeUrl(candidate.website),market:clean(candidate.market),
      score:candidate.score||{},confidence:clean(candidate.confidence)||"Low",matchedSignals:observed,
      recommendedOffer,buyerRoles:splitList(profile.decisionMakers),people:(candidate.people||[]).slice(0,5),
      whyNow,evidence,hypotheses,
      researchStatus:sourceCount?"complete":"error",researchAt:new Date().toISOString()
    };
  }

  function firstName(contact){return clean(contact?.firstName)||clean(contact?.name).split(" ")[0]||"";}
  function evidenceHook(dossier){
    const signal=dossier.matchedSignals?.[0]?.name;
    if(signal)return `public information connected to ${signal}`;
    const item=dossier.evidence?.[0];
    return item?.title?`the public information around ${item.title}`:"your company's recent public activity";
  }
  function buildOutreachDrafts(dossier={},contact={},profile={},tone="consultative"){
    const company=clean(dossier.company)||clean(dossier.domain)||"your company";
    const offer=clean(dossier.recommendedOffer)||splitList(profile.priorityOffers)[0]||"our work";
    const name=firstName(contact);
    const hello=name?`Hi ${name},`:"Hello,";
    const hook=evidenceHook(dossier);
    const sender=clean(profile.companyName)||"our team";
    let emailBody,linkedinMessage;
    if(tone==="direct"){
    ...
    return {tone:["consultative","direct","brief"].includes(tone)?tone:"consultative",emailSubject:`${company} — ${offer}`,emailBody,linkedinMessage:linkedinMessage.slice(0,899)};
  }

  function approveOutreachItem(item={},editedDrafts={},approvedAt=new Date().toISOString()){
    const drafts={
      emailSubject:clean(editedDrafts.emailSubject),
      emailBody:String(editedDrafts.emailBody??"").trim(),
      linkedinMessage:String(editedDrafts.linkedinMessage??"").trim(),
      tone:clean(editedDrafts.tone||item?.drafts?.tone||"consultative")
    };
  ...
  function normalizeEvidence(item={}){
    const url=normalizeUrl(item.url);if(!url)return null;
    return {url,title:clean(item.title),description:clean(item.description),text:String(item.text||"").slice(0,6000),date:clean(item.date),sourceType:["Official","Public","Discovery"].includes(clean(item.sourceType))?clean(item.sourceType):"Public"};
  }
  function normalizeItem(item={}){
    const domain=clean(item.domain).toLowerCase().replace(/^www\./,"");
    const researchStatus=RESEARCH_STATUSES.has(item.researchStatus)?item.researchStatus:"idle";
    const dossier=item.dossier&&typeof item.dossier==="object"?{
      ...item.dossier,company:clean(item.dossier.company),domain:clean(item.dossier.domain)||domain,website:normalizeUrl(item.dossier.website),market:clean(item.dossier.market),
      recommendedOffer:clean(item.dossier.recommendedOffer),buyerRoles:splitList(item.dossier.buyerRoles),whyNow:clean(item.dossier.whyNow),
      evidence:(item.dossier.evidence||[]).map(normalizeEvidence).filter(Boolean).slice(0,15),hypotheses:(item.dossier.hypotheses||[]).map(clean).filter(Boolean).slice(0,8),people:(item.dossier.people||[]).slice(0,5)
    }:null;
    return {
      domain,company:clean(item.company||dossier?.company),researchStatus,researchAt:clean(item.researchAt),dossier,
      selectedPersonId:clean(item.selectedPersonId),drafts:{tone:clean(item.drafts?.tone)||"consultative",emailSubject:clean(item.drafts?.emailSubject),emailBody:String(item.drafts?.emailBody||"").slice(0,12000),linkedinMessage:String(item.drafts?.linkedinMessage||"").slice(0,3000)},
      approved:Boolean(item.approved),approvedAt:clean(item.approvedAt),contactedAt:clean(item.contactedAt)
    };
  }
  function normalizeOutreachState(value={}){
    const input=value&&typeof value==="object"?value:{};
    return {selectedDomain:clean(input.selectedDomain).toLowerCase().replace(/^www\./,""),items:(Array.isArray(input.items)?input.items:[]).slice(0,50).map(normalizeItem).filter(x=>x.domain)};
  }
