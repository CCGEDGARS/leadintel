(function(root,factory){
  ...
  function buildAttentionItems({model=[],tasks=[],unsaved=false,runtimeErrors=[],now=Date.now()}={}){
    const items=[];
    ...
    current?.steps?.filter(step=>!step.complete&&!step.optional).forEach(step=>items.push({id:`stage-${current.id}-${clean(step.id,80)}`,severity:'important',title:clean(step.label||'Required step unfinished',120),detail:clean(step.action||`Finish this required step in ${current.name}.`),target:{type:'stage',id:current.id}}));
