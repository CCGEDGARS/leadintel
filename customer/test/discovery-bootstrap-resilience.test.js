...
test('Company Discovery bootstrap is not blocked by a module dependency graph', () => {
  assert.match(
    html,
    /<script defer src="discovery-ui\.js\?v=20260925-pipeline-choice-v1&sidebar-preservation=1&target-segments=1&opportunity-context=1&opportunity-map=1"><\/script>/,
...
test('Discovery shell cache keys match the runtime asset version', () => {
  ...
  for (const asset of ['discovery-engine.js', 'discovery-ui.js']) {
    assert.ok(html.includes(`<script defer src="${asset}?v=${runtimeVersion}${asset==='discovery-ui.js'?'&opportunity-context=1&opportunity-map=1':''}"></script>`), `${asset} must load the current Discovery release`);
  }
});
