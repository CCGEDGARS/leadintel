...
test('profile approval and Strategy navigation are separate user actions',()=>{
  ...
  assert.match(html,/profile-next-action[\s\S]*id="continue-market-strategy"[\s\S]*Continue to Strategy/);
...
test('profile approval card remains visible after approval',()=>{
  ...
  assert.match(runtime,/Your approved profile is ready for Strategy/);
