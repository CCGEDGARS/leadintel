...
test('workspace persistence boundary initializes before app state is loaded',()=>{
  const importMatch=app.match(/^import ['"]\.\/workspace-persistence\.js\?v=20260914-spinner-hard-stop-v1['"];?/m);
...
test('process map reuses the same startup-order persistence module URL',()=>{
  assert.match(processMap,/import ['"]\.\/workspace-persistence\.js\?v=20260914-spinner-hard-stop-v1['"]/);
});
