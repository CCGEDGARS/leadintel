...
test('managed Firecrawl 404 retries the direct proxy instead of failing every research query immediately',()=>{
  ...
  assert.match(processMap,/firecrawl-workspace-router\.js\?v=20260922-adaptive-evidence-v1/,'browser must receive the corrected router immediately');
});

test('Scrapling extraction fallback is exposed to the evidence pipeline',()=>{
  const router=fs.readFileSync(firecrawlRouterPath,'utf8');
  const app=fs.readFileSync(appPath,'utf8');
  assert.match(router,/X-LeadIntel-Extractor/);
  assert.match(router,/scrapling/);
  assert.match(app,/extractResearchPages/);
  assert.match(app,/buildAdaptivePlan/);
  assert.match(app,/buildQualityGate/);
});
