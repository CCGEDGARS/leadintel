const INTELLIGENCE_PROFILE_ASSET_VERSION='20260921-signal-focus-space-v1';
(function installIntelligenceProfileRuntime(root){
  ...
  function compactSignals(profile){const list=document.getElementById('recommended-signals');const panel=list?.closest('.signals-panel');if(!list||!panel)return;const active=(profile.recommendedSignals||[]).filter(x=>x.active!==false);panel.classList.add('intel-signal-summary-panel','intel-attention-panel');const title=panel.querySelector('.section-title');if(title)title.innerHTML=`<div><span class="eyebrow">Signal focus</span><h3>${active.length} active signal themes</h3><p>Approve the profile below, then edit signal weights and keywords in Market Strategy.</p></div>`;list.innerHTML=`<div class="intel-signal-summary" role="list" aria-label="Active signal themes">${active.slice(0,5).map((x,index)=>`<article class="intel-signal-theme" role="listitem"><span>${String(index+1).padStart(2,'0')}</span><strong>${escapeHtml(x.name)}</strong></article>`).join('')}</div>`;}
