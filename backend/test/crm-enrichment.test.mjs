...
async function fixture(role='owner'){
  ...
  return {env:{DB,APOLLO_API_KEY:'test-apollo-key'},token};
}
...
sqliteTest('CRM enrichment migration adds durable verification fields and request ledger',()=>{
  ...
  for(const field of ['workspace_id','company_id','contact_id','person_provider_id','status','credits_reserved','credits_used','phone_requested','response_summary_json'])assert.ok(requestColumns.includes(field),`${field} missing`);
});
