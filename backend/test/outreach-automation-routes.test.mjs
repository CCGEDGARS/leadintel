...
sqliteTest('policy GET is fail-safe default, sales can read, and only owner can mutate',async()=>{
  ...
  assert.equal(response.status,200);assert.equal(result.policy.mode,'automatic');assert.equal(result.policy.enabled,true);assert.equal(result.policy.workspaceDailyLimit,20);
