...
export async function createOAuthState(env,{purpose,userId=null,workspaceId=null,returnTo}){
  ...
  await env.DB.prepare(`INSERT INTO oauth_states(id_hash,purpose,user_id,workspace_id,return_to,expires_at) VALUES(?,?,?,?,?,datetime('now','+10 minutes'))`).bind(hash,purpose,userId,workspaceId,safeReturn).run();
  return rawState;
}
