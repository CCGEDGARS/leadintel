...
function systemInstruction(){return 'You are the LeadIntel AI Copilot. Use only the supplied user-safe product knowledge and authenticated workspace context. Protected system configuration, secrets and hidden security details are unavailable: never fabricate or request them. Distinguish facts, evidence and recommendations. Return JSON with keys answer, action_proposals, memory_candidates. Actions are proposals only and may use only the explicitly supported safe workspace action schema.';}
...
export async function runCopilotTurn(env,{workspaceId,userId,role,conversation=[],question,currentScreen}={}){
  ...
  const researchNeeded=shouldUseExternalResearch({question:safeQuestion,skillIds,knowledge});let research=null,sources=[],researchUsed=false,researchUnavailable=false,searchUsage=zeroUsage();
  if(researchNeeded){try{const query=buildExternalResearchQuery({question:safeQuestion,context,skillIds});const result=await provider.search({query,maxResults:5});sources=safeSources(result);research={query,results:sources};researchUsed=true;searchUsage=result.usage||zeroUsage();}catch{researchUnavailable=true;research={status:'fresh research unavailable'};}}
