...
export async function handleIntelligenceSourceRoute(request,env,cors={}){
  ...
  const match=url.pathname.match(/^\/api\/intelligence-sources\/([^/]+)$/);if(match){const id=decodeURIComponent(match[1]),row=await getOwnedSource(env,workspaceId,id);if(!row)return error('Source not found',404,cors);
    if(request.method==='PATCH'){
      ...
      await env.DB.prepare(`UPDATE intelligence_sources SET name=?,canonical_url=?,host=?,source_type=?,geography_json=?,auth_mode=?,auth_required=?,monitoring_enabled=?,mandatory=?,frequency=?,trigger_ids_json=?,authenticated_access_status=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND workspace_id=?`).bind(normalized.name,normalized.url,normalized.host,normalized.sourceType,JSON.stringify(normalized.geography),normalized.authMode,normalized.authRequired?1:0,Boolean(body.monitoring_enabled??body.monitoringEnabled??row.monitoring_enabled)?1:0,mandatory?1:0,normalized.frequency,JSON.stringify(normalized.triggerIds),row.auth_mode==='public'?'not_connected':clean(body.authenticated_access_status||body.authenticatedAccessStatus||row.authenticated_access_status,30),id,workspaceId).run();return json({source:sourceFromRow(await getOwnedSource(env,workspaceId,id))},200,cors);
    }
