...
function validateResult(text,context){
  ...
  result.warnings=[...new Set([...result.warnings,...context.warnings,...context.coverage.omitted.map(value=>`Source coverage: ${value}`)])];
...
async function analyze(env,{workspaceId,file,extraction,request,conversation=[],priorResult=null,outputPreferences={}}){
  const context=selectRelevantFileBlocks({request,extraction});
  ...
  for(let attempt=0;attempt<2;attempt++){
    ...
    try{return {result:validateResult(generated.text,context),provider:generated.provider,model:generated.model,usage};}
...
export async function runFileAnalysis(env,{workspaceId,userId,file,extraction,request,outputPreferences={}}){
  ...
  const saved=await env.DB.prepare(`INSERT INTO copilot_file_analyses(id,file_id,workspace_id,created_by,request,canonical_result_json,provider,model,usage_json,status,retained) SELECT ?,id,workspace_id,?,?,?,?,?,?,'completed',0 FROM copilot_files WHERE id=? AND workspace_id=? AND deleted_at IS NULL AND extraction_status IN ('complete','partial')`).bind(id,userId,request,JSON.stringify(generated.result),generated.provider,generated.model,JSON.stringify(generated.usage),file.id,workspaceId).run();
  if(!saved.meta?.changes)throw fail('File is no longer available for analysis',409);
  return {id,file_id:file.id,workspace_id:workspaceId,request,result:generated.result,status:'completed',retained:false};
}

export async function continueFileAnalysis(env,{workspaceId,userId,analysisId,message}){
  message=textRequest(message);
  if(!workspaceId||!userId)throw fail('Authenticated workspace context is required',403);
  const analysis=await env.DB.prepare(`SELECT * FROM copilot_file_analyses WHERE id=? AND workspace_id=? AND deleted_at IS NULL AND status='completed'`).bind(analysisId,workspaceId).first();
  ...
  const generated=await analyze(env,{workspaceId,file,extraction,request:message,conversation,priorResult:JSON.parse(analysis.canonical_result_json)});
  ...
  const writes=await env.DB.batch([
    env.DB.prepare(`INSERT INTO copilot_file_analysis_messages(id,analysis_id,role,content) SELECT ?,a.id,'user',? FROM copilot_file_analyses a JOIN copilot_files f ON f.id=a.file_id AND f.workspace_id=a.workspace_id WHERE a.id=? AND a.workspace_id=? AND a.status='completed' AND a.deleted_at IS NULL AND f.deleted_at IS NULL AND f.extraction_status IN ('complete','partial')`).bind(userMessageId,message,analysis.id,workspaceId),
    ...
    env.DB.prepare(`UPDATE copilot_file_analyses SET canonical_result_json=?,provider=?,model=?,usage_json=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND workspace_id=? AND EXISTS(SELECT 1 FROM copilot_file_analysis_messages WHERE id=?)`).bind(resultJson,generated.provider,generated.model,JSON.stringify(generated.usage),analysis.id,workspaceId,userMessageId)
  ]);
  if(!writes[2]?.meta?.changes)throw fail('File analysis is no longer available',409);
  return {id:analysis.id,file_id:file.id,workspace_id:workspaceId,request:analysis.request,result:generated.result,status:'completed',retained:Boolean(analysis.retained)};
}
